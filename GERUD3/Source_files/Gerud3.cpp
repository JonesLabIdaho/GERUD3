...
#include "G3LoadProgArray.h"
using namespace std;

// This file is part of Gerud3.  This version is version 3.1, updated to compile
...
{
	// For now just hard-code the filenames
	// Later, these can be passed as command-line arguments

	string tstr;
	string tempstring1, tempstring2;
	string tempprogfile, tempoutfile, tempallelefile;

	// set default filenames
	char* AlleleFrequencyFilename = new char[256];
	char* OutputFilename = new char[256];
	char* ProgenyArrayFilename = new char[256];
	bool verbose = true;

	tempallelefile = "Gerud3allelefrequencies.txt";
	tempoutfile = "Gerud3output.txt";
	tempprogfile = "Gerud3progenyarray.txt";

	int iii, jjj;
	for (iii = 1; iii < argc - 1; iii++)
	{
		tempstring1 = argv[iii];
		tempstring2 = argv[iii+1];
		if (tempstring1 == "-i")
			tempprogfile = tempstring2;
		if (tempstring1 == "-o")
	...
	// covert the filenames from strings to char arrays
	
	for (iii = 0; iii < static_cast<int>(tempprogfile.length()); iii++)
		ProgenyArrayFilename[iii] = tempprogfile[iii];
	ProgenyArrayFilename[iii] = '\0';
	...
	bool AllelesLoaded = false;
	if (AFflag != 0)
		AllelesLoaded = true;

	int PAflag;
	PAloader paloader;
	PAflag = paloader.LoadProgArray(ProgenyArrayFilename);

	if (verbose && AllelesLoaded)
	{
		cout << "\nAllele Frequencies Loaded!";
		cout << "\nFile Contents:";
		for (iii = 0; iii < AlleleFreq.NumberLoci; iii++)
		{
			cout << "\n" << AlleleFreq.LocusName[iii] << "\n";
	...
	if (verbose && !AllelesLoaded)
		cout << "\nAllele frequency file load unsuccessful!";
	...
	if (PAflag == 0)
	{
		...
		o_file << "Error Loading Progeny Array";
		o_file.close();

		return 0;
	}

	// Output progeny array

	if (verbose)
	{
		cout << "\nProgeny Array Successfully Loaded!";
		cout << "\nFile Contents:\n";

		if (paloader.KnownMother)
			cout << "Known Mother\n";
		else
			cout << "Unknown Mother\n";

		cout << paloader.NumberEmbryos << " Embryos\n";
		cout << paloader.NumberLoci << " Loci\n";
		cout << "Loci:\t";
		for (iii = 0; iii < paloader.NumberLoci; iii++)
			cout << paloader.locusname[iii] << "\t";
		cout << "\n";

		if (paloader.KnownMother)
		{
			cout << paloader.motherid << "\t";
			for (jjj = 0; jjj < paloader.NumberLoci; jjj++)
			{
				cout << paloader.motherallele1[jjj] << "/" << paloader.motherallele2[jjj] << "\t";
			}
			cout << "\n";
		}

		for (iii = 0; iii < paloader.NumberEmbryos; iii++)
		{
			cout << iii+1 << "\t";
			for (jjj = 0; jjj < paloader.NumberLoci; jjj++)
				cout << paloader.embryoallele1[jjj][iii] << "/" << paloader.embryoallele2[jjj][iii] << "\t";
			cout << "\n";
		}
	}
	...
	ParentSet *BestSolutionsOld;
	 ...
	 // most of these variables are not used by Gerud3
	 ...
	 PossDads = new Dad[2000000];  // allow up to 2,000,000 possible dads
	 ...
	 // Need to make sure the locus names correspond to the locus names in the progeny array
	 // The official loci under consideration are those in the progeny array

	 int namecounter;
	 NumberLoci = paloader.NumberLoci;
	 for (i = 0; i < NumberLoci; i++)
	 {
		 namecounter = 0;
		 for (j = 0; j < static_cast<int>(paloader.locusname[i].length()); j++)
		 {
			 if (paloader.locusname[i][j] != '\r' && paloader.locusname[i][j] != '\t' &&
				 paloader.locusname[i][j] != '\n' && paloader.locusname[i][j] != ' ')
			 {
				 Loci[i].Name[namecounter] = paloader.locusname[i][j];
				 namecounter++;
			 }
		 }
		 Loci[i].Name[namecounter] = '\0';
	 }

	 NumFreqLoci = AlleleFreq.NumberLoci;
	 int matchinglocus;
	 bool namematch;
	 bool LocusMatchingFlag = true;
	 for (i = 0; i < NumberLoci; i++)
	 {
		 matchinglocus = 1000;
		 for (j = 0; j < NumFreqLoci; j++)
		 {
			 char name1[100];
			 namecounter = 0;
			 for (k = 0; k < static_cast<int>(AlleleFreq.LocusName[j].length()); k++)
			 {
				 if (AlleleFreq.LocusName[j][k] != '\r' && AlleleFreq.LocusName[j][k] != '\t' &&
					 AlleleFreq.LocusName[j][k] != '\n' && AlleleFreq.LocusName[j][k] != ' ')
				 {
					 name1[namecounter] = AlleleFreq.LocusName[j][k];
					 namecounter++;
				 }
			 }
			 name1[namecounter] = '\0';
			 ...
			 namecounter = 0;
			 namematch = true;
			 while (name1[namecounter] != '\0')
			 {
				 if (name1[namecounter] != Loci[i].Name[namecounter])
					 namematch = false;
				 namecounter++;
			 }
		 ...
		 if (matchinglocus < NumFreqLoci)
		 {
			 Loci[i].NumberofAlleles = AlleleFreq.NumberAlleles[matchinglocus];
			 for (j = 0; j < Loci[i].NumberofAlleles; j++)
			 {
				 Loci[i].AlleleName[j] = AlleleFreq.AlleleName[j][matchinglocus];
				 Loci[i].AlleleFreq[j] = AlleleFreq.AlleleFreq[j][matchinglocus];
			 }
		 }
		 else
		 {
			 AllelesLoaded = false;
			 LocusMatchingFlag = false;
		 }
	 }
	 NumLociLoaded = NumberLoci;

	 if (verbose && !LocusMatchingFlag)
	 {
		 cout << "\nLocus names in progeny array were not found in allele frequency file.";
		 cout << "\nNote that locus names are case sensitive.";
	 }
	 ...
	 for (i = 0; i < NumberLoci; i++)
	 {
		 ...
		 for (j = 0; j < Loci[i].NumberofAlleles; j++)
		 {
			 cout << Loci[i].AlleleName[j] << "\t" << Loci[i].AlleleFreq[j] << "\n";
		 }
	 }
	 ...
	 // This part will not be necessary because Gerud doesn't need these parameters
	 ...
	 */
	 ...
	 // Pass the information about the progeny array to old Gerud3 variables

	 NumberLoci = paloader.NumberLoci;
	 NumLoci = NumberLoci;
	 MomKnown = paloader.KnownMother;

	 // This is the main loop, which goes through all the proposed runs and the reps of these runs
	 // Except that this is Gerud, so it only runs through everything once for the actual dataset
	 ...
	 for (run = 0; run < 1; run++)  // this loop is only used in Gerudsim, so in Gerud it runs through only once
	 {
			...
			for (iteration = 0; iteration < 1; iteration++)  // this loop is only used in Gerudsim, so in Gerud it goes through once
			{
				MinDadsOneLocus = 10;
				BigDadsSolved = false;
				...
				//  Create Array Here -------------------------------------------------------
				// This step is not necessary for GERUD, because there's a real array 
				/*
				Dad tempDads[10];
				...
				for (i = 0; i < NumberEmbryos; i++)
				{
					...
					for (j = 0; j < RealMaleNumber[run]; j++)
					{
						...
						if (rndnum < sumnum && EmbryoRealFather[i] == 99)
						{
							...
							for (j = 0; j < NumberLoci; j++)
							{
								cout << MomAllele1[j] << "/" << MomAllele2[j] << "\t";
							}// end of j loop
							cout << "\n";
							...
							for (i = 0; i < RealMaleNumber[run]; i++)
							{
								...
								for (j = 0; j < NumberLoci; j++)
								{
									cout << RealDad[i].Allele1[j] << "/" << RealDad[i].Allele2[j] << "\t";
								}// end of j loop
								cout << EmsSampFromMale[i];
								cout << "\n";
							}  // end of i loop
							for (i = 0; i < NumberEmbryos; i++)
							{
								...
								for (j = 0; j < NumberLoci; j++)
								{
									cout << EmbryoAllele1[j][i] << "/" << EmbryoAllele2[j][i] << "\t";
								} // end of j loop
								cout << EmbryoRealFather[i] + 1 << "\n";
							}  // end of i loop */
				...
				// Pass the rest of the progeny array information here

				NumberEmbryos = paloader.NumberEmbryos;
				NumberLoci = paloader.NumberLoci;
				MomKnown = paloader.KnownMother;
				NumNotSolved = 0;
				...
				for (i = 0; i < NumberEmbryos; i++)
				{
					for (k = 0; k < NumberLoci; k++)
					{
						EmbryoAllele1[k][i] = paloader.embryoallele1[k][i];
						EmbryoAllele2[k][i] = paloader.embryoallele2[k][i];
					}
				}
				...
				if (MomKnown)
				{
					for (k = 0; k < NumberLoci; k++)
					{
						MomAllele1[k] = paloader.motherallele1[k];
						MomAllele2[k] = paloader.motherallele2[k];
					}
				}
				
				// free up paloader memory.
				...
				// Do Maternal Alleles Routine --------------------------------------------------------------------
					...
					// If the mother is known, make sure that all offspring are compatible with the mom
					...
					if (MomKnown)
					{
						...
						for (i = 0; i < NumberEmbryos; i++)
						{
							for (j = 0; j < NumberLoci; j++)
							{
								if (!compatible(Mothers[0].Allele1[j], Mothers[0].Allele2[j], EmbryoAllele1[j][i], EmbryoAllele2[j][i]))
									bcompat = false;
							} 
						}
					...
					if (MomKnown && NumCompMoms == 0)
					{
						o_file.open(OutputFilename);
						o_file << "\nThe maternal genotype is incompatible with at least one progeny genotype.\n";
						o_file << "Note that GERUD does not allow missing data, so any offspring with missing\n";
						o_file << "genotypes should be removed from the progeny array.";
						o_file.close();
					...
					{
						o_file.open(OutputFilename);
						o_file << "\nThis progeny array is not consistent with any single maternal genotype.\n";
						o_file << "In other words, based on Mendelian Laws, this progeny array does not\n";
						o_file << "contain only full- and half-siblings (maybe there's a genotyping error).";
						o_file.close();
				...
				// Do Big Search ------------------------------------------------------------
					...
					if (verbose && !DadsSolved)
						cout << "\nTesting single father solutions...";
					...
					if (verbose && !DadsSolved)
						cout << "\nTesting three-father solutions...";
					...
					if (verbose && !DadsSolved)
						cout << "\nTesting seven-father solutions...";
					...
					if (verbose && !DadsSolved)
						cout << "\nTesting eight-father solutions...";
				...
				if (BigDadsSolved) 
				{
					...
					// --------------------------------------------------------------------------------------
					// This check results routine is only relevant to GerudSim, so it's commented out here.
					/*
					bool UsedRDad[6];
					bool UsedSDad[8];
					bool DadGenoMatch[6];
					bool DadExactMatch[6];
					int NumberCorrect;
					bool tFlag;
					bool tFlagSlvd;
				...
				// This next part is also unnecessary for Gerud3.
				/*
					if (CorrectDadNumber)
						CumCorrDadNum++;
					if (OverDadNumber)
	 ...
	 o_file.open(OutputFilename);
	 o_file << "Results from Gerud3:\n";
	 o_file << "Input File:\t" << ProgenyArrayFilename << "\n";
	 o_file << "Total Number of Solutions:\t" << TotalNumberSolutions << "\n";

	 if (ErrorPP)
		o_file << "There were PP errors. Results are suspect.\n";

	 if (ErrorGP)
		o_file << "There were GP Errors. Results are suspect.\n";

	 if (TotalNumberSolutionsError)
		o_file << "There were TNS Errors. Results are suspect.\n";
   
	 if (MAnumberError)
		o_file << "There were MAnumber Errors. Results are suspect.\n";

	 if (NumWinDadError)
		o_file << "There were NumWinDad Errors. Results are suspect.\n";

	 o_file << "Parent\t";
	 for (i = 0; i < NumberLoci; i++)
		 o_file << Loci[i].Name << "\t";
	 o_file << "#prog\t" << "probability_score";
	 
	 if (!BigDadsSolved)
		 o_file << "\nGERUD cannot solve this progeny array!";
	 else
	 {
		 for (i = 0; i < NumBestDads; i++)
		 {
			 ...
			 o_file << "\nMother";
			 for (j = 0; j < NumberLoci; j++)
				 o_file << "\t" << BestSolutions[i].MoAll1[j] << "/" << BestSolutions[i].MoAll2[j];
			 for (k = 0; k < BestSolutions[i].NumberFathers; k++)
			 {
				 o_file << "\nFath_" << k + 1;
				 for (j = 0; j < NumberLoci; j++)
					 o_file << "\t" << BestSolutions[i].FaAll1[j][k] << "/" << BestSolutions[i].FaAll2[j][k];
				 o_file << "\t" << BestSolutions[i].FaEmb[k];
				 if (TotalNumberSolutions > 1)
					o_file << "\t" << BestDadsPriority[i];
			 } // k
		 } // i
	 } // else
